...
EnginePlayVideo::EnginePlayVideo() 
{
...
void EnginePlayVideo::sloTimerRender()
{
	while (m_render) {
		uint8_t* idY = nullptr;
		uint8_t* idU = nullptr;
		...
		int videoW = 0;
		int videoH = 0;
		auto videodecode = m_frame_engine->getVidioDecode();
...
EnginePlayAudio::EnginePlayAudio(AnalyzeFrameEngine* engine, QObject* parent) 
	: QObject(parent), m_frame_engine(engine)
{

	if (m_frame_engine != NULL) {
		// 重采样参数 输出参数
		auto audio_decoder = m_frame_engine->getAudioDecoder();
...
EnginePlayAudio::~EnginePlayAudio()
{
	SDL_CloseAudio();
}
...
void EnginePlayAudio::sdl_audio_callback(void* opaque, Uint8* stream, int len) {
	...
	auto decoderptr = is->m_frame_engine->getAudioDecode();
...
EnginePlayer::EnginePlayer(QObject* parent /* = nullptr */) : QObject(parent) {
	m_videoplayer = new EnginePlayVideo();
}
...
void EnginePlayer::Play(AnalyzeFrameEngine* engine)
{
	// 开始播放音频
